# Relative Time Bundle – Demos

Demos for Composer package [nowo-tech/relative-time-bundle](https://packagist.org/packages/nowo-tech/relative-time-bundle) ([source on GitHub](https://github.com/nowo-tech/RelativeTimeBundle)) on Symfony 7 and 8.
...
Each demo mounts the bundle root at `/var/relative-time-bundle` in the container. The demo’s `composer.json` uses a path repository pointing there, so you must run `make up-*` (or `docker-compose up`) from the **bundle repository root** so that `../..` resolves to the bundle.
...
- **nowo-tech/relative-time-bundle** (from path)
