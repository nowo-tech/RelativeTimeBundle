...
The entrypoint uses `FRANKENPHP_MODE`: `classic` copies `Caddyfile.dev`; `worker` (default) keeps the baked-in worker Caddyfile. Default ports are **8007** for `demo/symfony8` and **8008** for `demo/symfony8` (see each demo’s `.env` or `PORT`).
